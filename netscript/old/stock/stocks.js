...
export function addStock(symbolValue, forecastValue, priceValue, ammountValue) {
    ...
    let stock = {
        ...
        getPurchaseAmmount: function(money) {
            let purchaseAmmount = Math.floor((money / this.price) * 1);
            if(purchaseAmmount + this.ammount > this.maxAmmount){
                purchaseAmmount = this.maxAmmount - this.ammount;
            }
            logger.debugArr("PURCHASE AMMOUNT", 
            ["AMMOUNT " + purchaseAmmount, 
            "MONEY: " + money, 
