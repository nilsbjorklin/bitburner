...
export async function main(ns) {
  ns.disableLog("ALL");
  while (true) {
    for (let i = 0; i < ns.hacknet.numNodes(); i++) await upgradeHacknetNode(ns, i);
...
async function upgradeLevel(ns, index, upgraded) {
  let levelCost = ns.hacknet.getLevelUpgradeCost(index, 1);
  if (levelCost < checkMoney(ns)) {
    await ns.sleep(100);
