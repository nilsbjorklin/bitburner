...
export async function main(ns) {
    if (ns.args[0].toUpperCase() === "START") {
        ...
        let lockedServers = 0;
        let unavailableServers = 0;
        let ramServers = 0;
        for (let i = 1; i < serverList.length; i++)
            addServer(ns, serverList[i]);
        ...
        ns.tprint(`NOT STARTED SERVERS: ${lockedServers+unavailableServers}, 
            NOT ENOUGH RAM: ${ramServers} SERVERS, 
            COULD NOT UNLOCK: ${lockedServers} SERVERS, 
            HACKING LEVEL NOT HIGH ENOUGH: ${unavailableServers} SERVERS`);
...
async function startHackScript(ns, server) {
    ...
    if (threads > 0) {
        ...
        tprint(`STARTED '${hackScript}' WITH ${threads} THREADS ON SERVER '${server.serverName}'`);
    }
    return "STARTED";
}
...
async function tryHack(ns, server) {
    if (server.hackLevel <= hackingLevel) {
        ...
        ns.print(`HACKING LEVEL NOT HIGH ENOUGH(${hackingLevel}) REQUIRED: ${server.hackLevel}`);
        return "LEVEL";
    }
}
